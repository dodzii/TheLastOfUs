...
public class GameMapScene extends Scene { 
	...
	public GameMapScene(Parent root) {
		...
		Label l = new Label("Controlled Hero");l.setStyle("-fx-font-family: Papyrus, fantasy ; -fx-font-size: 30; -fx-text-fill: White");
		...
		ScaleTransition scaleTransition = new ScaleTransition(Duration.seconds(0.85), leftUp);
		...
		leftDown = new VBox();
		Label ld = new Label("Acquired Heroes");ld.setStyle("-fx-font-family: Papyrus, fantasy ; -fx-font-size: 30; -fx-text-fill: White");
		...
		scrollPanel.setStyle("-fx-background-color: transparent; -fx-control-inner-background: transparent;");
			...
			if(e.getCode() == KeyCode.UP){
				try {
					...
					if(tmpBefore > tmpAfter){
						AlertBox a = new AlertBox("You Have Stepped On A Trap Cell!!");
					...
					if(Game.checkGameOver()){
						if(Game.checkWin()){
				            ...
				            GameWonScene o = new GameWonScene(new StackPane());o.setFill(Color.BLACK);
				            ...
				            fadeOut.setFromValue(1.0);
				            fadeOut.setToValue(0.05);
				            GameLostScene o = new GameLostScene(new StackPane());o.setFill(Color.BLACK);
				            FadeTransition fadeIn = new FadeTransition(Duration.seconds(0.75), o.getRoot());
				            fadeIn.setFromValue(0.0);
				            fadeIn.setToValue(1.0);
				            fadeOut.setOnFinished(e3 -> {	            	
				...
				try {
					...
					if(Game.checkGameOver()){
						if(Game.checkWin()){
							GUI.window.setScene(new GameWonScene(new VBox()));
						}
						else{
							GUI.window.setScene(new GameLostScene(new VBox()));
						}
					}
		...
		main = new BorderPane();
		main.setStyle("-fx-background: transparent;-fx-background-image: url('images/MapBackground.png'); "+"-fx-background-repeat: no-repeat;-fx-background-size: 100% 100%;");
		main.setCenter(map);
		main.setLeft(left);
	...
	public static void updateMap(){
		...
		for(int i = 0 ; i<15 ; i++){
			for(int j = 0 ; j<15 ; j++){
				if(Game.map[j][i] instanceof CharacterCell && ((CharacterCell)Game.map[j][i]).getCharacter() instanceof Fighter){
					FighterButton cell = new FighterButton(((Hero)((CharacterCell)Game.map[j][i]).getCharacter()));
					map.add(cell,i,14-j);
				}
				else if(Game.map[j][i] instanceof CharacterCell && ((CharacterCell)Game.map[j][i]).getCharacter() instanceof Medic) {
					MedicButton cell = new MedicButton(((Hero)((CharacterCell)Game.map[j][i]).getCharacter()));
					map.add(cell,i,14-j);
				}
				else if(Game.map[j][i] instanceof CharacterCell && ((CharacterCell)Game.map[j][i]).getCharacter() instanceof Explorer) {
					ExplorerButton cell = new ExplorerButton(((Hero)((CharacterCell)Game.map[j][i]).getCharacter()));
					map.add(cell,i,14-j);
				}
				else if(Game.map[j][i] instanceof CharacterCell && ((CharacterCell)Game.map[j][i]).getCharacter() instanceof Zombie) {
					ZombieButton cell = new ZombieButton(Game.map[j][i].isVisible(),((Zombie)((CharacterCell)Game.map[j][i]).getCharacter()));
					map.add(cell,i,14-j);
				}
				else if(Game.map[j][i] instanceof CollectibleCell && ((CollectibleCell)Game.map[j][i]).getCollectible() instanceof Supply) {
					SupplyButton cell = new SupplyButton(Game.map[j][i].isVisible());
					map.add(cell,i,14-j);
				}
				else if(Game.map[j][i] instanceof CollectibleCell && ((CollectibleCell)Game.map[j][i]).getCollectible() instanceof Vaccine) {
					VaccineButton cell = new VaccineButton(Game.map[j][i].isVisible());
					map.add(cell,i,14-j);
				}
				else{// if(Game.map[i][j] instanceof CharacterCell && ((CharacterCell)Game.map[i][j]).getCharacter()==null) {
					EmptyButton cell = new EmptyButton(Game.map[j][i].isVisible());
					map.add(cell,i,14-j);
				}
			}
		}
